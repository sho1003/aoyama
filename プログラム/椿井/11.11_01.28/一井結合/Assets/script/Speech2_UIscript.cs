...
public class Speech2_UIscript : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        player2 = transform.root.gameObject;
    ...
    void Update()
    {
        ...
        //一番近いプレイヤーを取得
        GameObject otherplayer = SearchTag(player2, "Player2");
        if (otherplayer != null)
        {
            //プレイヤーとのベクトルを取得
            Vector3 vec = otherplayer.transform.position - player2.transform.position;
            //絶対値を取る
        ...
        //ワールド座標をスクリーン座標に変換
        var screenPos = Camera.main.WorldToScreenPoint(player2.transform.position + offset);
